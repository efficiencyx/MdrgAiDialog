...
public static class ModelCatalog {
  ...
  /// the provider can't be reached (the UI then falls back to <see cref="Curated"/> + custom entry).
  /// </summary>
  ...
  /// <summary>
  /// Extracts model ids from the various shapes providers return:
  /// <c>{"data":[{"id":..}]}</c> (OpenAI), <c>{"models":["..",{"id"/"name":..}]}</c> (Jun/Ollama),
  /// or a bare array of strings. Embedding models are dropped (not usable for chat).
  /// </summary>
  ...
  /// <summary>Known-good defaults per provider, used when the live list can't be fetched.</summary>

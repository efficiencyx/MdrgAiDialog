...
/// <summary>
/// Streams AI dialogue to a TTS server sentence-by-sentence and plays it back through
/// a Unity AudioClip/AudioSource. Requests are pipelined (chunk N+1 generates while
/// chunk N plays) and playback order matches text order.
/// </summary>
[MonoSingleton]
...
public class TtsManager : MonoBehaviour {
  ...
  // Re-reads [Tts] and rebuilds the client, so settings-panel changes apply without a restart
  public void Reload() {
  ...
  // Starts a new spoken message, stopping any audio left over from the previous one
  ...
  // Holds back audio queued after this point until ReleaseBarrier. Used for
  // #!flow.SplitMessage so speech pauses for the user's click instead of reading ahead
  ...
  // Releases the oldest pending barrier (the user passed the split point)
